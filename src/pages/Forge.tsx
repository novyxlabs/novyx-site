  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const response = await fetch('https://formspree.io/f/xbdyeprl', {
        ...
        body: JSON.stringify({ email, source: 'forge-waitlist' })
      })
              ...
              <>
                <form onSubmit={handleSubmit} className="flex gap-2">
                  <input
                    ...
                    required
                    disabled={loading}
                    className="flex-1 px-4 py-3 rounded-lg bg-[#0D1117] border border-[#30363D] focus:border-accent-forge focus:outline-none transition-colors disabled:opacity-50"
                  />
                  <button
                    type="submit"
                    disabled={loading}
                    className="px-6 py-3 rounded-lg bg-accent-forge hover:bg-pink-600 text-white font-medium transition-colors disabled:opacity-50 disabled:cursor-not-allowed"
                  >
